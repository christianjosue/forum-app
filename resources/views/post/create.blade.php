...
<form action="{{ url('post') }}" method="post">
  ...
  <div class="container mt-3 border border-gray bg-dark text-white rounded-lg w-25">
      <div class="form-group p-3 m-0">
        <h4 class="mb-3 mt-2">Create Post</h4>
        <label for="title">Title</label>
        <input value="{{ old('title') }}" required type="text" class="form-control" minlength="1" maxlength="50" id="title" name="title">
        @error('title')
      ...
      <div class="form-group p-3 m-0">
        ...
        <textarea required class="rounded-lg w-100" id="message" name="message" minlength="5" maxlength="100" rows="4">{{ old('message') }}</textarea>
        @error('message')
          <div class="alert alert-danger m-0 mt-2 p-1 text-center">{{ $message }}</div>
        @enderror
      </div>
      <button type="submit" class="btn btn-primary ml-3 mb-4">Add post</button>
